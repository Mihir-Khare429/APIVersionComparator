...
const logMapper = (data) => {
  ...
  filteredLogs.forEach((log) => {
    ...
    if (body) {
      const direction =
        body?.HttpDirection !== undefined
          ? body.HttpDirection
          : getRequestDirection(mt);
      ...
      if (key != undefined) {
        if (directionDictionary.hasOwnProperty(key)) {
          ...
          directionToAdd =
            directionAlreadyPresent == "Outgoing" ? "Incoming" : "Outgoing";
          console.log(
            "Direction to add",
            directionToAdd,
            "Already Present",
            directionAlreadyPresent
          );
        } else {
        ...
        if (
          ...
          directionToAdd != directionDictionary[key]
        ) {
